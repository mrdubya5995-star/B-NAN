class EJS_GameManager {
    ...
    getRetroArchCfg() {
        // PATCHED by B-NAN (see docs/BUILD.md's "Patches to vendored code"
        // section) -- upstream hardcodes audio_latency=64 and
        // video_vsync=true here, completely unconditionally, with no
        // public EJS_* hook to override either one (retroarchOpts, the
        // only real override mechanism, only ever gets populated from a
        // core's own bundled core.json -- there's no equivalent EJS_
        // global, confirmed by reading the rest of this bundle). Forcing
        // vsync locks video to the display's flat 60Hz against systems
        // that don't run at exactly that rate (a GBA is ~59.7Hz), and the
        // audio pipeline has to absorb that drift somehow -- it comes out
        // as periodic crackle. 64ms of audio buffer is also aggressive
        // enough that ordinary main-thread jitter (GC pauses, layout)
        // causes audible underruns. This is very likely the real,
        // persistent root cause of choppy audio reported repeatedly
        // through B-NAN's whole development -- every earlier fix
        // (removing B-NAN's own forced setVSync call, killing a periodic
        // IndexedDB write during play, hiding a compositing decorative
        // overlay, broader WASM threading) was real and worth doing, but
        // none of them touched THIS, because it's baked into EmulatorJS's
        // own vendored source, not anything B-NAN's own code was setting.
